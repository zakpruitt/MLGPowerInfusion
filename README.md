...
- **Banner:** uses Blizzard's aura frame (`AuraContainer`), which the game shows by itself while PI is on you, so it also works in combat. It stays up for the whole PI window, and a new random message is picked each time you leave combat.
